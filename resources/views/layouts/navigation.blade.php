<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Location Switcher + Settings Dropdown -->
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                <x-dropdown align="right" width="56">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->currentLocation?->name ?? __('No Location') }}</div>
                    ...
                    <x-slot name="content">
                        @foreach (Auth::user()->locations as $location)
                            @if ($location->id === Auth::user()->current_location_id)
                                <span class="block w-full px-4 py-2 text-start text-sm leading-5 text-gray-400 dark:text-gray-500">
                                    {{ $location->name }} ({{ ucfirst($location->pivot->role ?? '') }})
                                </span>
                            @else
                                <form method="POST" action="{{ route('locations.switch', $location) }}">
                                    @csrf

                                    <x-dropdown-link :href="route('locations.switch', $location)"
